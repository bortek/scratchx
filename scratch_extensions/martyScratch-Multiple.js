...
loadScript("https://bortek.github.io/scratchx/js/marty.js?v=20180403");
...
//(function(ext) {
    ...
    ext.walk = function(numsteps, step_length, turn, step_time, callback){
        ...
        for (m in martys){
            martys[m].walk(parseInt(numsteps), parseInt(turn), step_time*1000, parseInt(step_length));
        }
        if (ext.blocking_mode === true){
    ...
    ext.walk_forward = function(numsteps, callback){
        for (m in martys){
            martys[m].walk(parseInt(numsteps), 0, 1800, 40);
        }
        if (ext.blocking_mode === true){
    ...
    ext.turn = function(direction, numsteps, callback){
      ...
      for (m in martys){
        martys[m].walk(parseInt(numsteps), turn, 1300, 0);
      }
      if (ext.blocking_mode === true){
    ...
    ext.lean = function(direction, move_time, callback){
        for (m in martys){
            martys[m].lean(direction, 50, move_time*1000);
        }
        if (ext.blocking_mode === true){
    ...
    ext.circle_dance = function(direction, move_time, callback){
        for (m in martys){
            martys[m].circle_dance(direction, move_time*1000);
        }
        if (ext.blocking_mode === true){
            setTimeout(callback, move_time*1000);
        } else {
    ...
    ext.eyes = function(position, callback){
        ...
        for (m in martys){
            martys[m].move_joint(8, eyepos[position], 100);
        }
        if (ext.blocking_mode === true){
    ...
    ext.sidestep = function(side, num_steps, step_time, step_length, callback){
        for (m in martys){
            martys[m].sidestep(side, parseInt(num_steps), step_time*1000, parseInt(step_length));
        }
        if (ext.blocking_mode === true){
    ...
    ext.moveJoint = function(joint, angle, move_time, callback){
        for (m in martys){
            martys[m].move_joint(jointID[joint], angle, move_time*1000);
        }
        if (ext.blocking_mode === true){
            setTimeout(callback, move_time*1000);
        } else {
    ...
    ext.play_sound = function(start_freq, end_freq, duration, callback){
        for (m in martys){
            martys[m].play_sound(parseInt(start_freq), parseInt(end_freq), parseInt(duration*1000));
        }
        callback();
    }
    ...
    ext.move_leg = function(leg, direction, callback){
        ...
        for (m in martys){
            martys[m].move_joint(jointID[joint], 10*mult, 750);
        }
        setTimeout(callback, 750);
    }
    ...
    ext.lower_leg = function(callback){
        for (m in martys){
            var left_knee = martys[m].get_sensor("mp" + jointID['left knee']);
            var right_knee = martys[m].get_sensor("mp" + jointID['right knee']);
            if (left_knee === null || right_knee === null){
                setTimeout(ext.lower_leg, 200, callback);
                return;
            }
            if (right_knee > 0 && right_knee > left_knee){
                martys[m].move_joint(jointID['right knee'], left_knee, 500);
            } else {
                martys[m].move_joint(jointID['left knee'], right_knee, 500);
            }
        }
    ...
    ext.get_sensor = function(sensor_name, callback){
        ...
        for (m in martys){
            var response = martys[m].get_sensor(sensor_name);
            break;
        }
        if (response === null){
    ...
    ext.getGPIO = function(gpio_id, callback){
        ...
        for (m in martys){
            var response = martys[m].get_sensor("gpio" + gpio_id);
            break;
        }
        if (response === null){
    ...
    ext.get_accel = function(axis, callback){
        ...
        for (m in martys){
            var response = martys[m].get_sensor("acc" + axisID[axis]);
            break;
        }
        if (response === null){
    ...
    ext.get_battery = function(callback){
        for (m in martys){
            var response = martys[m].get_sensor("battery");
            break;
        }
        if (response === null){
    ...
    ext.get_prox_sensor = function(callback){
        for (m in martys){
            var response = martys[m].get_sensor("prox");
            break;
        }
        if (response === null){
    ...
    ext.stop = function(stop_type, callback){
        for (m in martys){
            martys[m].stop(stopType[stop_type]);
        }
        callback();
    }
/*
...
function selectorExtension(ext){
    ...
    var descriptor = {
        blocks: [
            ...
            [' ', 'Rescan', 'rescan'],
            [' ', 'Select all Martys', 'add_all_martys']
        ],
        menus: {
            martys : martyNames,
        }
